...
export function UsersPanel({ initialUsers, initialMessages }: { initialUsers: User[]; initialMessages: ContactMessage[] }) {
  ...
  async function deleteMessage(messageId: string) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok || !data.ok) throw new Error(data.error || 'No se pudo eliminar la consulta');
      setMessages((data.messages as ContactMessage[]) ?? []);
      setStatus('Consulta eliminada.');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'No se pudo eliminar la consulta');
    } finally {
      setLoadingMessageId(null);
    }
  }
              ...
              <tr key={user.id}>
                ...
                <td>
                  {user.role === 'admin' ? (
                    ...
                    <div className="inline-actions users-payment-actions">
                      <select
                        className="users-payment-select"
            ...
            {messages.map((message) => (
              <article key={message.id} className="contact-admin-card">
                ...
                <div className="contact-admin-actions">
                  ...
                  <button
                    className="btn btn-danger btn-small contact-delete-btn"
                    type="button"
